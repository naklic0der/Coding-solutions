...
void dfs(int s, int color=0) {
	if(!c[s]) { // is it already visited
		if(c[s]^color) { // do both(s and its child) nodes have same color
			cout << "IMPOSSIBLE" << endl;
			exit(0);
		}
		return;   // because there is no point to explore this node, as it is already colored
	}
	// Let's explore this new node
	c[s] = color;
	for(int v: adj[s]) {
		dfs(v,color^1);
	}
